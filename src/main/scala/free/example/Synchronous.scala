...
/** This is a "synchronous" interpreter, using [[Try]] as our Monad
  */
case class Synchronous(flags: FeatureFlags, httpClient: HttpRequest => HttpResponse)
    extends ~>[Operation, Try]:
  def apply[A](operation: Operation[A]): Try[A] =
    operation match {
      case GetFeatureFlags                => Try(flags)
      case GetCustomerById(customerId)    =>
        Try {
          val response = httpClient(Get("/openBet"))
          UserData(customerId, response.body.contains("selfExcluded"))
        }
      case CheckGamStop(customerId)       =>
        Try {
          val response = httpClient(Get("/gamstop"))
          if response.code == 200 then
            Some(UserData(customerId, response.body.contains("selfExcluded")))
          else None
        }
      case WriteSelfExclusion(cust, flag) =>
        Try {
